...
public class WindowInHandRenderer {
	...
	public void renderWindow(PoseStack poseStack, SubmitNodeCollector collector, float sideMult, int light, ItemStack itemStack) {
		...
		if(!toplevel.framebuffer.isValid()) return;
		...
		float offset = (Math.min(wscale, 0.5f) - 0.5f) * (0.14f / 0.5f);
		...
		Vec3 origin = new Vec3(-0.5, 0.5, 0.0);
		Vec3 spanX = new Vec3(1.0, 0.0, 0.0);
		Vec3 spanY = new Vec3(0.0, -1.0, 0.0);

		RenderUtils.renderFramebuffer(toplevel.framebuffer, poseStack, collector, true, origin, spanX, spanY);
	}
